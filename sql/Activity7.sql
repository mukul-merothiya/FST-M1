REM   Script: Session 7
REM   avg, max, min, cunt
...
select order_no from orders ordered by order_date asc;
...
select avg(purchase_amount) as "Average Sum" from orders ;

select max(purchase_amount) as "Max Amount" from orders ;

select min(purchase_amount) as "Max Amount" from orders ;
